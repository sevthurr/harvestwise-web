  ...
  return (
    <Card>
      <div className="flex items-center justify-between mb-2">
        <SectionLabel>{t("farmer.settings.section_text_size", {}, "Text Size")}</SectionLabel>
        <span className="text-[14px] font-semibold text-[var(--hw-green-700)] -mt-3">
          {t(`farmer.settings.text_size_${textSize}`, {}, TEXT_SIZE_OPTIONS.find((o) => o.value === textSize)?.label)}
        </span>
      </div>
      ...
      <input
        ...
        onChange={(e) => {
          ...
          if (opt) {
            ...
            showToast(t("farmer.settings.toast_text_size_updated", {}, "Text size updated."));
          }
        }}
      ...
      <div className="flex justify-between mt-2">
        {TEXT_SIZE_OPTIONS.map((opt) => (
          <span
            key={opt.value}
            className={`text-[12px] font-medium ${textSize === opt.value ? "text-[var(--hw-green-700)]" : "text-black"}`}
          >
            {t(`farmer.settings.text_size_${opt.value}`, {}, opt.label)}
          </span>
        ))}
      </div>
    </Card>
