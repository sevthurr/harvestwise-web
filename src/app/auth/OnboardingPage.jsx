...
const SELLING_OPTIONS = [
  { id: "farmgate", label: "To a buyer using farmgate price" },
  { id: "market", label: "Directly in the market" },
  { id: "trader", label: "Through a trader" },
  { id: "unsure", label: "Not sure yet" }
];
const LanguageSVG = () => <svg viewBox="0 0 108 84" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-28 h-20 mx-auto">
    {
  /* Globe fill */
}
  ...
  /* Center vertical meridian */
    ...
    {
  /* Speech bubble — solid fill */
}
    {
  /* Tail pointing toward globe (lower-left) */
}
...
const LocationSVG = () => <svg viewBox="0 0 100 90" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-24 h-20 mx-auto">
...
const CropsSVG = () => <svg viewBox="0 0 110 80" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-28 h-20 mx-auto">
    {
  /* Soil */
}
...
const SellingSVG = () => <svg viewBox="0 0 110 75" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-28 h-20 mx-auto">
    {
  /* Stall base */
}
  ...
  /* Awning scallops */
    ...
    {[16, 28, 40, 52, 64, 76, 88].map((x, i) => <semicircle key={i} />)}
    <path d="M8 38 Q14 44 20 38 Q26 44 32 38 Q38 44 44 38 Q50 44 56 38 Q62 44 68 38 Q74 44 80 38 Q86 44 92 38 Q98 44 102 38" stroke="#22c55e" strokeWidth="2" fill="none" />
    {
  /* Support poles */
}
...
const CompleteSVG = () => <svg viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-28 h-28 mx-auto">
...
const LocationModal = ({ onAllow, onCancel, detecting }) => <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
...
const StepCard = ({ children }) => <div className="w-full max-w-lg bg-white rounded-2xl border border-[var(--hw-neutral-200)] shadow-[0_2px_16px_0_rgba(0,0,0,0.07)] p-8">
    {children}
  </div>;
const OptionChip = ({ label, selected, onClick, icon }) => <button
  type="button"
  onClick={onClick}
  className={`flex items-center justify-between gap-3 w-full px-4 py-3.5 rounded-xl border text-[15px] font-medium transition-all text-left ${selected ? "bg-[var(--hw-green-50)] border-[var(--hw-green-700)] text-[var(--hw-green-900)]" : "bg-white border-[var(--hw-neutral-200)] text-[var(--hw-neutral-800)] hover:bg-[var(--hw-neutral-50)]"}`}
>
    <span className="flex items-center gap-2.5 min-w-0">
      {icon}
      {label}
    </span>
    {selected && <div className="flex-shrink-0 w-5 h-5 rounded-full bg-[var(--hw-green-700)] flex items-center justify-center">
        <Check className="w-3 h-3 text-white" />
      </div>}
  </button>;
const NavButtons = ({ step, onBack, onContinue, onSkip, continueLabel = "Continue" }) => <div className="space-y-2 pt-2">
    <div className="flex gap-2">
      {step > 1 && onBack && <button
  type="button"
  onClick={onBack}
  className="flex items-center justify-center gap-1 px-5 h-12 border border-[var(--hw-neutral-200)] text-[15px] font-medium text-[var(--hw-neutral-700)] rounded-xl hover:bg-[var(--hw-neutral-50)] transition-colors"
>
  ...
  type="button"
  onClick={onContinue}
  className="flex-1 h-12 flex items-center justify-center gap-1.5 bg-[var(--hw-green-700)] text-white text-[15px] font-semibold rounded-xl hover:bg-[var(--hw-green-800)] transition-colors"
>
        {continueLabel}
  ...
  type="button"
  onClick={onSkip}
  className="w-full text-center text-[14px] text-[var(--hw-neutral-400)] hover:text-[var(--hw-neutral-600)] transition-colors py-1"
>
      Skip for now
    </button>
  </div>;
const TOTAL = 4;
const Step1 = ({ data, onChange, onContinue, onSkip }) => <StepCard>
...
const Step2 = ({ data, onChange, onContinue, onBack, onSkip }) => {
  ...
  />}
    ...
    /* Initial choice */
        ...
        {data.locationMode === null && <div className="mt-6 flex gap-3">
            <button
    type="button"
  ...
  >
    ...
    /* Location detected banner */
        ...
        {data.locationMode === "auto" && <div className="mt-5 p-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between">
  ...
  >
    ...
    /* Form fields */
        ...
        {(data.locationMode === "manual" || data.locationMode === "auto") && <div className="mt-5 space-y-4">
...
const Step3 = ({ data, onChange, onContinue, onBack, onSkip }) => {
  ...
  const setVariety = (name, variety) => {
    onChange({ crops: { ...data.crops, [name]: variety } });
  };
  return <StepCard>
    ...
    return <button
      ...
      className={`flex items-center gap-2.5 h-12 px-3 text-[14px] font-medium rounded-xl border transition-all text-left ${selected ? "bg-[var(--hw-green-50)] border-[var(--hw-green-700)] text-[var(--hw-green-900)]" : "bg-white border-[var(--hw-neutral-200)] text-[var(--hw-neutral-800)] hover:bg-[var(--hw-neutral-50)]"}`}
    >
      ...
      {
    /* Variety selectors for selected crops */
  }
      {Object.keys(data.crops).length > 0 && <div className="mt-5 space-y-3 pt-4 border-t border-[var(--hw-neutral-100)]">
    ...
    return <div key={cropName} className="flex items-center gap-3">
...
const Step4 = ({ data, onChange, onContinue, onBack, onSkip }) => <StepCard>
    ...
    <div className="mt-6 space-y-3">
      {SELLING_OPTIONS.map((opt) => <OptionChip
  key={opt.id}
  label={opt.label}
...
/>
    ...
    <div className="mt-6">
      <NavButtons step={4} onBack={onBack} onContinue={onContinue} onSkip={onSkip} continueLabel="Finish setup" />
    </div>
  </StepCard>;
...
const SetupComplete = ({ data, onDone }) => {
  ...
  return <StepCard>
    ...
    /* Summary */
      ...
      <div className="mt-6 space-y-0 divide-y divide-[var(--hw-neutral-100)]">
        {[
    { label: "Language", value: LANGUAGE_LABEL[data.language] },
  ...
  ].map((row) => <div key={row.label} className="flex justify-between gap-3 text-[14px] py-2.5">
...
function OnboardingPage() {
  ...
  const patch = (d) => setData((prev2) => ({ ...prev2, ...d }));
  ...
  return <div className="min-h-screen bg-white flex flex-col items-center px-4 py-12">
    ...
    /* Progress dots (steps 1–4 only) */
      ...
      {step < 5 && <div className="flex gap-1.5 mb-6">
          {[1, 2, 3, 4].map((s) => <div key={s} className={`h-1.5 rounded-full transition-all duration-300 ${s === step ? "w-7 bg-[var(--hw-green-700)]" : s < step ? "w-3 bg-[var(--hw-green-400)]" : "w-3 bg-[var(--hw-neutral-200)]"}`} />)}
        </div>}
      ...
      {step === 3 && <Step3 data={data} onChange={patch} onContinue={next} onBack={prev} onSkip={skip} />}
      {step === 4 && <Step4 data={data} onChange={patch} onContinue={next} onBack={prev} onSkip={skip} />}
