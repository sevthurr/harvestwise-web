  ...
  return <Card>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            <FieldLabel htmlFor="ud-mn" optional>Middle Name</FieldLabel>
            <input id="ud-mn" type="text" value={form.middleName} onChange={set("middleName")} placeholder="Optional" className={inputCls} />
          </div>
          <div>
            <FieldLabel htmlFor="ud-sfx" optional>Suffix</FieldLabel>
        ...
        {form.role === "DFTC" && <div>
            <FieldLabel htmlFor="ud-pos" optional>Position</FieldLabel>
            <input id="ud-pos" type="text" value={form.position} onChange={set("position")} placeholder="e.g. Market Monitoring Staff" className={inputCls} />
          </div>}
...
function AdminUserDetails() {
  ...
  if (!user) {
    return <div className="px-4 md:px-8 lg:px-10 py-5 max-w-[1240px] mx-auto">
      ...
      className="flex items-center gap-1.5 text-[14px] text-black hover:text-[var(--hw-green-700)] transition-colors mb-4"
    >
          <ChevronLeft className="w-4 h-4" />
          Back to User Accounts
        </button>
        <p className="text-[15px] text-black">User not found.</p>
      </div>;
  }
  ...
  const infoRow = (label, value) => <div key={label} className="flex items-start justify-between py-3 gap-4">
  ...
  return <div className="px-4 md:px-8 lg:px-10 py-5 pb-24 md:pb-8 max-w-[1240px] mx-auto space-y-4">
    ...
    type="button"
    onClick={() => navigate("/admin/system")}
    className="flex items-center gap-1.5 text-[14px] text-black hover:text-[var(--hw-green-700)] transition-colors"
  >
        <ChevronLeft className="w-4 h-4" />
        Back to User Accounts
      </button>
    ...
    /* Header */
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-bold text-black">{user.name}</h1>
          <p className="text-[15px] text-black mt-0.5">{user.role} · {user.status}</p>
        </div>
    ...
    className="h-11 px-5 flex items-center text-[14px] font-semibold text-red-600 border border-red-200 rounded-xl hover:bg-red-50 transition-colors"
  >
              Disable account
            </button> : <GreenBtn onClick={() => setEnableModal(true)}>Enable account</GreenBtn>}
        </div>
      </div>
    ...
    /* Account Information */
      ...
      {!editing && <Card>
          ...
          <div className="divide-y divide-[var(--hw-neutral-100)]">
            ...
            {infoRow("Suffix", user.suffix !== "None" ? user.suffix : "\u2014")}
    ...
    /* Disable modal */
      ...
      {disableModal && <Modal title="Disable account?" onClose={() => setDisableModal(false)}>
  ...
  >
    ...
    /* Enable modal */
      ...
      {enableModal && <Modal title="Enable account?" onClose={() => setEnableModal(false)}>
          <p className="text-[14px] text-black mb-5">
            This user will be able to access HarvestWise again.
          </p>
          <div className="flex gap-2 justify-end">
