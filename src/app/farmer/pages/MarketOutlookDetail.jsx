...
const STATUS_CLS = {
  "Favorable": { color: "text-emerald-700", bg: "bg-emerald-50", border: "border-l-emerald-500" },
  "Balanced": { color: "text-blue-600", bg: "bg-blue-50", border: "border-l-blue-500" },
  "Watch closely": { color: "text-amber-700", bg: "bg-amber-50", border: "border-l-amber-500" },
  "Cautious": { color: "text-red-700", bg: "bg-red-50", border: "border-l-red-500" }
};
...
function getGlanceFactors(commodityId, name) {
  ...
  if (!outlook) return [];
  const priceVariant = outlook.priceBehavior === "Rising" ? "neutral" : outlook.priceBehavior === "Falling" ? "unfavorable" : "neutral";
  ...
  return [
    {
      name: "Price Outlook",
      classification: outlook.priceBehavior === "Rising" ? "Neutral" : outlook.priceBehavior === "Falling" ? "Unfavorable" : "Neutral",
      detail: `${name} prices may ${outlook.priceBehavior === "Rising" ? "stay near or above recent levels" : outlook.priceBehavior === "Falling" ? "continue to fall" : "remain near recent levels"}.`,
      action: "View prices",
    ...
    },
    {
      ...
      detail: `${name} arrivals remained in the ${arrivalClass === "Upper Middle" ? "upper half" : arrivalClass === "Low" ? "below-normal range" : "lower half"} of historical records during the last 4 weeks.`,
...
const WATCH_TEXT = {
  kamatis: "If you plan to plant Kamatis now, monitor prices and avoid expanding your area too quickly.",
  talong: "If you plan to plant Talong, check prices and costs before finalizing your planting area.",
  repolyo: "If you are considering Repolyo, wait for price signals to improve before committing to planting.",
  atsal: "If you are planning Atsal, confirm profitability and monitor supply patterns before expanding.",
  carrots: "If you plan to plant Carrots, monitor arrival volumes and price direction before planting.",
  ampalaya: "If you are planning Ampalaya, confirm the profitability outlook and check if supply remains low.",
  kalabasa: "If you plan to plant Kalabasa, review your break-even price carefully before committing.",
  pipino: "If you plan to plant Pipino, check costs and market prices before finalizing your area.",
  lettuce: "Consider waiting for better conditions before planting Lettuce. Check if oversupply eases.",
...
function MarketOutlookDetailPage() {
  ...
  const statusLabel = STATUS_MAP[outlook.status] ?? "Balanced";
  const statusCls = STATUS_CLS[statusLabel];
